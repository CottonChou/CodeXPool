...
PKG_DIR="$ARTIFACT_DIR/Copool-$STAMP"
mkdir -p "$PKG_DIR"
cp "$BIN_PATH" "$PKG_DIR/"

if [[ -n "${CODESIGN_IDENTITY:-}" ]]; then
  echo "[2/4] Codesigning binary"
  codesign --force --timestamp --options runtime --sign "$CODESIGN_IDENTITY" "$PKG_DIR/Copool"
else
  echo "[2/4] Skip codesign (CODESIGN_IDENTITY not set)"
fi

ZIP_PATH="$ARTIFACT_DIR/Copool-$STAMP.zip"
...
if [[ -n "${NOTARY_PROFILE:-}" ]]; then
  ...
  xcrun stapler staple "$PKG_DIR/Copool" || true
else
