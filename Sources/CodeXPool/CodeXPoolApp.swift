...
@main
struct CodeXPoolApp: App {
    private let container: AppContainer
    @StateObject private var trayModel: TrayMenuModel
    #if os(macOS)
    @NSApplicationDelegateAdaptor(CodeXPoolAppDelegate.self) private var appDelegate
    #elseif os(iOS)
    @UIApplicationDelegateAdaptor(CodeXPoolAppDelegate.self) private var appDelegate
    #endif
    ...
    private func makeMenuBarSymbolImage() -> NSImage? {
        guard let base = NSImage(systemSymbolName: "figure.pool.swim", accessibilityDescription: "CodeXPool") else {
            return nil
        }
...
#if os(macOS)
@MainActor
private final class CodeXPoolAppDelegate: NSObject, NSApplicationDelegate {
...
#elseif os(iOS)
@MainActor
private final class CodeXPoolAppDelegate: NSObject, UIApplicationDelegate {
    func application(
...
private func handleRemoteNotification(_ userInfo: [AnyHashable: Any]) -> Bool {
    ...
    if notification.subscriptionID == CloudKitCurrentAccountSelectionSyncService.pushSubscriptionID {
        NotificationCenter.default.post(name: .codeXPoolCurrentAccountSelectionPushDidArrive, object: nil)
        return true
    }

    if notification.subscriptionID == CloudKitAccountsSyncService.pushSubscriptionID {
        NotificationCenter.default.post(name: .codeXPoolAccountsSnapshotPushDidArrive, object: nil)
        return true
    }
