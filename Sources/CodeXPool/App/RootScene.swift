...
private struct ProviderIconView: View {
    ...
    private var nsImage: NSImage? {
        switch provider {
        case .codex: return ProviderBrandIcon.codexIcon()
        case .claude: return ProviderBrandIcon.claudeIcon()
        }
    }
        ...
        HStack(spacing: 0) {
            ForEach(Array(tabs.enumerated()), id: \.element) { index, tab in
                Button {
                    ...
                    Group {
                        #if canImport(AppKit)
                        if let providerType = tab.providerType {
                            ProviderIconView(provider: providerType)
                        } else {
...
private extension AppTab {
    #if canImport(AppKit)
    var providerType: ProviderIconView.ProviderType? {
        switch self {
